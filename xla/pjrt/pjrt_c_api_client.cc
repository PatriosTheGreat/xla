...
StatusOr<struct CompileOptions> PjRtCApiExecutable::GetCompileOptions() const {
  auto* c_api = pjrt_c_api();
  auto* executable = c_executable();
  ...
  RETURN_STATUS_IF_ERROR(c_api->PJRT_Executable_GetCompileOptions(&args),
                         c_api);
  absl::Cleanup cleanup = [&args]() { (*args.result_deleter)(&args); };
  ...
  if (!options_proto.ParseFromArray(args.compile_options,
    ...
    return tsl::errors::InvalidArgument(
        "PJRT_Executable_GetCompileOptions: failed to deserialize "
        "CompileOptionsProto");
  }
...
// Initializes `PJRT_DeviceTopology_Compile_Args`, which will be used to call
// API PJRT_DeviceTopology_Compile().
static StatusOr<std::unique_ptr<PjRtExecutable>> InitializeArgsAndCompileAot(
  ...
  args.struct_size = PJRT_DeviceTopology_Compile_Args_STRUCT_SIZE;
  args.priv = nullptr;
  if (client == nullptr) {
    args.client = nullptr;
  } else {
  ...
  RETURN_STATUS_IF_ERROR(c_api->PJRT_DeviceTopology_Compile(&args), c_api);
