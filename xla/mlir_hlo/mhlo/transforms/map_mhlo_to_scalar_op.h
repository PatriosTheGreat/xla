...
inline Value mapMhloOpToStdScalarOp<mhlo::MaxOp>(Location loc,
  ...
  auto yieldLhs = [&](OpBuilder& b, Location loc) {
    b.create<scf::YieldOp>(loc, lhs);
  };

  auto yieldRhs = [&](OpBuilder& b, Location loc) {
    b.create<scf::YieldOp>(loc, rhs);
  };

  return b->create<scf::IfOp>(loc, cond, yieldLhs, yieldRhs).getResult(0);
}
