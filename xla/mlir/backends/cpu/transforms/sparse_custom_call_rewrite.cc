...
class SparseCustomCallToPackRewriter
  ...
  LogicalResult matchAndRewrite(mhlo::CustomCallOp op,
    ...
    const StringRef sparsePackCallName = "sparse_tensor_pack";
    const StringRef sparseUnpackCallName = "sparse_tensor_unpack";

    if (op.getCallTargetName().equals(sparsePackCallName)) {
      assert(op.getInputs().size() == 2 && "Need two arrays (data/indices)");
      assert(op.getResults().size() == 1 && "Must be packing into one tensor");
      Value retSpTensor = op.getResults()[0];
      rewriter.replaceOpWithNewOp<sparse_tensor::PackOp>(
          op, retSpTensor.getType(), op.getInputs()[0], op.getInputs()[1]);
      return success();
    } else if (op.getCallTargetName().equals(sparseUnpackCallName)) {
      llvm_unreachable("not yet implemented");
    }
    // Return failure on unmatched call target.
    return failure();
  };
};
